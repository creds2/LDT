...
```{r int_pass, echo=FALSE}
pass_od_dom <- read_sf("../../data/clean/od_flights_pass.gpkg")
ctrs <- c("United Kingdom","Irish Republic","Guernsey","Isle of Man","Jersey")
pass_od_dom <- pass_od_dom[pass_od_dom$airport1_country %in% ctrs & 
                             pass_od_dom$airport2_country %in% ctrs,]
pass_od_dom <- pass_od_dom[,c("airport1","airport1_country", "airport2","airport2_country","X2018","pass_km_2018")]
pass_od_dom <- pass_od_dom[!is.na(pass_od_dom$X2018),]
pass_od_dom <- pass_od_dom[pass_od_dom$X2018 > 0,]
pass_od_dom$lenght_km <- as.numeric(st_length(pass_od_dom)) / 1000

m5 <- tm_shape(pass_od_dom[order(pass_od_dom$X2018),]) +
  tm_lines(col = "X2018", 
           lwd = "X2018",
...
tmap_save(m5, filename = "domestic_OD.png")
