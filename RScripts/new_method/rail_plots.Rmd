...
#Brussles
iso_brus_60 <- read_sf("../../data/isochrones/brussels60.geojson")
iso_brus_120 <- read_sf("../../data/isochrones/brussels120.geojson")
iso_brus_180 <- read_sf("../../data/isochrones/brussels180.geojson")
iso_brus_240 <- read_sf("../../data/isochrones/brussels240.geojson")
iso_brus_300 <- read_sf("../../data/isochrones/brussels300.geojson")
iso_brus <- rbind(iso_brus_300,iso_brus_240,iso_brus_180, iso_brus_120, iso_brus_60)
...
iso_brus$duration <- iso_brus$duration + 120 # Brussle 2h away

iso_lyon <- st_make_valid(iso_lyon)
iso_barc <- st_make_valid(iso_barc)
iso_Koln <- st_make_valid(iso_Koln)
iso_turin <- st_make_valid(iso_turin)
iso_brus <- st_make_valid(iso_brus)

iso_final <- rbind(iso_lond, iso_brus, iso_paris_nore, iso_lyon, iso_poit, iso_stras, iso_amst, iso_saar, iso_edin, iso_Koln,iso_barc, iso_turin, iso_san, iso_mad, iso_vero, iso_stut,iso_frank,
...
newlines <- sf::st_make_valid(newlines)
...
#png("../../plots/rail_map.png", width = 2186, height = 2016)
m2 <- tm_shape(bounds, bbox = bbox) +
  tm_lines(col = "#424242", lwd = 0.5) +
  tm_shape(rail) +
  ...
  tm_shape(places) +
  tm_dots(size = 0.01) +
  tm_text("name", just = "bottom", size = 0.8, xmod = 0.5, col = "#403d3d") +
  tm_layout(legend.position = c(0.80,0.01),
            ...
            legend.title.size=1,
            frame = TRUE)
tmap_save(m2, filename = "../../plots/rail_map.png")
...
sf_use_s2(TRUE)
airports_inter_zone <- airports_inter[st_buffer(iso_single, 3000000),]
airports_inter_zone <- st_join(airports_inter_zone, iso_final)

bbox <- st_bbox(st_buffer(iso_single, 1000000))

airports_dom$pass_2019_total <- rowSums(st_drop_geometry(airports_dom[,c("pass_2019_TRUE","pass_2019_FALSE")]), na.rm = TRUE)
airports_inter_zone$pass_2019_total_mln <- airports_inter_zone$pass_2019_total / 1e6

m5 <- tm_shape(airports_inter_zone, bbox = bbox) +
  tm_dots(col = "hours",
             size = "pass_2019_total_mln",
          ...
          title.size="Passengers\nto/from UK\nmillions\n(2019)") +
  tm_layout(legend.bg.color = "white", 
            legend.bg.alpha = 0.5) +
  #tm_shape(airports_dom[!is.na(airports_dom$pass_2019_TRUE),]) +
  tm_shape(airports_dom[airports_dom$pass_2019_total > 0,]) +
  tm_dots(shape = 18, scale = 2, col = "blue") +
  tm_shape(bounds) +
  tm_lines() +
  tm_layout(legend.position = c(0.05,0.3),
            frame = FALSE, outer.margins = 0)
tmap_save(m5, filename = "../../plots/airports_study_area_Inter4.png")
...
origin_cols <- tmaptools::get_brewer_pal("Dark2", 2)
...
names(airports_summary1) <- c("region2","pass_2019_FALSE","pass_2019_TRUE","pass_km_2019_FALSE", "pass_km_2019_TRUE","emissions_2019_FALSE","emissions_2019_TRUE")
airports_summary2 <- airports_summary2[,names(airports_summary1)]

airports_summary <- rbind(airports_summary1, airports_summary2)

write.csv(airports_summary,"../../data/study_area_summary_v4.csv")
...
pass_od_rail <- read_sf("../../data/clean/od_emissions_2021_rail_newkgco2.gpkg")
pass_od_rail <- st_drop_geometry(pass_od_rail)
pass_od_rail <- pass_od_rail[,c("airport1","airport1_country","airport2","airport2_country","length_rail_km","kgco2_railpsngr")]
...
pass_od$emissions_rail_2019 = pass_od$pass_2019 * pass_od$kgco2_railpsngr 
#pass_od$emissions_rail_2019_old = pass_od$pass_km_rail_2019 * 0.00446 #BEIS KgCO2e 2021 per pass km

# plot(pass_od$emissions_rail_2019_old, pass_od$emissions_rail_2019, xlim = c(0,3e7), ylim=c(0,3e7))
# abline(0,1)
...
rail_sf <- read_sf("../../data/clean/od_emissions_2021_rail_newkgco2.gpkg")
rail_sf_geom <- readRDS("../../data/clean/rail_routes_newkgco2.Rds")
rail_sf <- st_drop_geometry(rail_sf)
rail_sf_geom = rail_sf_geom[,c("route")]

rail_sf$route = paste0(rail_sf$airport1,"_",rail_sf$airport1_country,"_", rail_sf$airport2,"_", rail_sf$airport2_country)

rail_sf = left_join(rail_sf, as.data.frame(rail_sf_geom), by = "route")
